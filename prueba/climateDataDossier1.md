...
Vamos a cargar los archivos de datos del pasado para las tres variables: `precip`, `tmax` y `tmin`. Para cada pixel y por cada variable tenemos varios atributos: NA - ***p-value*** NA - pertenencia a espacio natural
...
Algunos datos: - `nrow(p_precip_sn[p_precip_sn$tau>0,])` pixeles (`(nrow(p_precip_sn[p_precip_sn$tau>0,])/nrow(p_precip_sn))*100` %) presentan una tendencia positiva. Existen `nrow(filter(p_precip_sn, tau > 0, p_value < 0.05))` pixeles con \(\tau > 0\) y `pvalue < 0.05`, lo que representa un `(nrow(filter(p_precip_sn, tau > 0, p_value < 0.05))/nrow(filter(p_precip_sn, tau > 0)))*100` % del total de los pixels con \(\tau\) positivo y un `(nrow(filter(p_precip_sn, tau > 0, p_value < 0.05))/nrow(p_precip_sn))*100` % del total de pixeles para Sierra Nevada. - `nrow(p_precip_sn[p_precip_sn$tau<0,])` pixeles (`(nrow(p_precip_sn[p_precip_sn$tau<0,])/nrow(p_precip_sn))*100` %) presentan una tendencia negativa. Existen `nrow(filter(p_precip_sn, tau < 0, p_value < 0.05))` pixeles con \(\tau < 0\) y `pvalue < 0.05`, lo que representa un `(nrow(filter(p_precip_sn, tau < 0, p_value < 0.05))/nrow(filter(p_precip_sn, tau < 0)))*100` % del total de los pixels con \(\tau\) negativo y un `(nrow(filter(p_precip_sn, tau < 0, p_value < 0.05))/nrow(p_precip_sn))*100` % del total de pixeles para Sierra Nevada.
...
![plot of chunk precip\_tau\_plot](./climateDataDossier1_files/figure-markdown_github/precip_tau_plot.png)
...
![plot of chunk precip\_tau\_cart](./climateDataDossier1_files/figure-markdown_github/precip_tau_cart.png)
...
![plot of chunk precip\_combined\_plot](./climateDataDossier1_files/figure-markdown_github/precip_combined_plot.png)
